...
export function parsePaginationParams(
  ...
  const page = Math.max(1, Number(searchParams.get("page") ?? String(defaultPage)));
  const pageSize = Math.min(
    Math.max(1, Number(searchParams.get("pageSize") ?? String(defaultPageSize))),
    maxPageSize
  );
  const sort = (searchParams.get("sort") ?? defaultSort).trim();
  const order: "asc" | "desc" =
    (searchParams.get("order") ?? defaultOrder).toLowerCase() === "asc" ? "asc" : "desc";
