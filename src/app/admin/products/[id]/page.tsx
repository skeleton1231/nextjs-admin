  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/admin/products/${id}`} className="rounded border px-3 py-1 text-sm hover:bg-muted">
          预览(Modal)
        </Link>
        <Link href={`/admin/products/${id}/edit`} className="rounded border px-3 py-1 text-sm hover:bg-muted">
          编辑
        </Link>
      </div>
      ...
      <button type="button" onClick={() => router.back()} className="rounded border px-3 py-1 text-sm hover:bg-muted">
        返回
      </button>
    </div>
  );
